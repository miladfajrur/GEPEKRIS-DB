...
import { hasTabAccess, hasPermission, ROLE_PRESETS } from "../lib/permissions";
...
export default function Dashboard() {
  const { user, logout, isDirectAccessMode, setDirectAccessMode, deleteCurrentAccount } = useAuth();
  ...
  const [activeTab, setActiveTab] = useState(() => {
    if (user?.permissions?.allowedTabs && user.permissions.allowedTabs.length > 0) {
      return user.permissions.allowedTabs.includes('overview') ? 'overview' : user.permissions.allowedTabs[0];
    }
  ...
  const ketuaJemaat = "Pdt. Yohanes Santoso, M.Th.";
  ...
  // Granular permissions derived from logged-in user
  const canViewProfile = hasPermission(user, 'canViewMemberProfile');
  const canCreateMember = hasPermission(user, 'canCreateMember');
  const canEditMember = hasPermission(user, 'canEditMember');
  const canDeleteMember = hasPermission(user, 'canDeleteMember');
  const canExportData = hasPermission(user, 'canExportData');
  const canImportData = hasPermission(user, 'canImportData');
  ...
  useEffect(() => {
    ...
    if (user?.username && !hasGreeted) {
      ...
      else if (user.username === "anabk") message = `Selamat ${time} Pengurus Jemaat`;
      else if (user.username === "BEM") message = `Selamat ${time} Pengurus`;
  ...
  const handleExportExcel = async () => {
    try {
      ...
      XLSX.writeFile(workbook, "Buku_Induk_GEPEKRIS_TRETES.xlsx");
  ...
  const handleExportPDF = async () => {
    try {
      ...
      doc.text("Laporan Induk Data GEPEKRIS TRETES", 40, 48);
      
      doc.setFontSize(11);
      doc.setFont("helvetica", "normal");
      doc.setTextColor(100, 100, 100);
      doc.text(`Total Keseluruhan Jemaat: ${members.length} Jiwa  |  Tanggal Unduh: ${formatDateDDMMYYYY(new Date().toISOString())}`, 40, 68);
      ...
      doc.save("Laporan_Induk_GEPEKRIS_TRETES.pdf");
  ...
  useEffect(() => {
    if (user?.permissions?.allowedTabs && user.permissions.allowedTabs.length > 0) {
      if (!user.permissions.allowedTabs.includes(activeTab)) {
        setActiveTab(user.permissions.allowedTabs[0]);
      }
    } else if (user?.username === 'BEM') {
  ...
  const renderNavLinks = () => {
    return (
      <>
        ...
        {(hasTabAccess(user, 'reports_umum') || hasTabAccess(user, 'reports_remaja') || hasTabAccess(user, 'reports_anak') || hasTabAccess(user, 'reports')) && (
          <>
            <div className="w-full px-4 pt-2 pb-1 text-[10px] font-bold uppercase tracking-wider text-white/40">
              Data Kebaktian
            </div>

            {hasTabAccess(user, 'reports_umum') && (
              <button 
                onClick={() => handleTabClick("reports_umum")}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all focus:outline-none ${activeTab === 'reports_umum' ? 'bg-white/10 text-white font-medium opacity-100' : 'opacity-60 hover:opacity-100'}`}
              >
                {activeTab === 'reports_umum' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <Church className="w-4 h-4 text-blue-400" />}
                Kebaktian Umum
              </button>
            )}

            {hasTabAccess(user, 'reports_remaja') && (
              <button 
                onClick={() => handleTabClick("reports_remaja")}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all focus:outline-none ${activeTab === 'reports_remaja' ? 'bg-white/10 text-white font-medium opacity-100' : 'opacity-60 hover:opacity-100'}`}
              >
                {activeTab === 'reports_remaja' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <Users className="w-4 h-4 text-purple-400" />}
                Kebaktian Remaja
              </button>
            )}

            {hasTabAccess(user, 'reports_anak') && (
              <button 
                onClick={() => handleTabClick("reports_anak")}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all focus:outline-none ${activeTab === 'reports_anak' ? 'bg-white/10 text-white font-medium opacity-100' : 'opacity-60 hover:opacity-100'}`}
              >
                {activeTab === 'reports_anak' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <Sparkles className="w-4 h-4 text-amber-400" />}
                Kebaktian Anak
              </button>
            )}

            {hasTabAccess(user, 'reports') && (
              <button 
                onClick={() => handleTabClick("reports")}
                className={`w-full flex items-center gap-3 px-4 py-2.5 rounded-lg text-sm transition-all focus:outline-none ${activeTab === 'reports' ? 'bg-white/10 text-white font-medium opacity-100' : 'opacity-60 hover:opacity-100'}`}
              >
                {activeTab === 'reports' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <Layers className="w-4 h-4 text-emerald-400" />}
                Ringkasan Terpadu
              </button>
            )}
          </>
        ...
        {(hasTabAccess(user, 'finance') || hasTabAccess(user, 'map') || hasTabAccess(user, 'stats') || hasTabAccess(user, 'worship')) && (
          <div className="w-full h-px bg-white/10 my-1"></div>
        )}

        {hasTabAccess(user, 'finance') && (
          <button 
          ...
          >
            {activeTab === 'finance' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <Wallet className="w-4 h-4 text-emerald-400" />}
            Manajemen Keuangan
          </button>
        )}

        {hasTabAccess(user, 'map') && (
          <button 
        ...
        {(hasTabAccess(user, 'users') || hasTabAccess(user, 'settings')) && (
          <div className="w-full h-px bg-white/10 my-1"></div>
        )}

        {hasTabAccess(user, 'users') && (
          <button 
            onClick={() => handleTabClick("users")}
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-lg text-sm transition-all focus:outline-none ${activeTab === 'users' ? 'bg-white/10 text-white font-medium opacity-100' : 'opacity-60 hover:opacity-100'}`}
          >
            {activeTab === 'users' ? <span className="w-2 h-2 rounded-full bg-blue-400"></span> : <ShieldCheck className="w-4 h-4 text-purple-400" />}
            Manajemen Akun & Akses
          </button>
        )}

        {hasTabAccess(user, 'settings') && (
          <button 
    ...
    <div className={`flex h-[100dvh] w-full font-sans text-slate-800 dark:text-slate-100 overflow-hidden bg-slate-50 dark:bg-slate-900 ${isDarkMode ? 'dark' : ''}`}>
      ...
      <aside className={`fixed md:relative z-50 bg-slate-900 dark:bg-slate-950 text-white flex-shrink-0 h-full transition-all duration-300 ease-in-out ${isSidebarOpen ? "translate-x-0 w-64" : "-translate-x-full md:translate-x-0 w-64 md:w-0"} md:overflow-hidden`}>
        <div className="w-64 p-6 h-full flex flex-col">
          <div className="flex items-center gap-3 mb-10">
            <div className="h-10 w-10 bg-blue-600 rounded-xl flex items-center justify-center shrink-0 text-white shadow-md">
              <Church className="w-6 h-6" />
            </div>
            <div>
              <h1 className="text-xs font-bold leading-tight opacity-70 uppercase tracking-widest">Buku Induk</h1>
              <p className="text-sm font-semibold">GEPEKRIS TRETES</p>
            </div>
            {/* Mobile close button */}
            <button 
          ...
          <div className="mt-auto pt-6 border-t border-white border-opacity-10 flex flex-col gap-4">
            <div className="flex items-center justify-between gap-2 overflow-hidden">
              <div className="flex items-center gap-3 min-w-0 flex-1">
                <div className="w-10 h-10 rounded-full bg-blue-700 flex items-center justify-center font-bold uppercase shrink-0 overflow-hidden text-white">
                  {(user?.displayName || user?.username)?.[0] || 'U'}
                </div>
                <div className="min-w-0 flex-1">
                  <p className="text-sm font-medium line-clamp-1 break-words" title={user?.displayName || user?.username}>
                    {user?.displayName || (user?.username === 'anabk' ? 'Pengurus Jemaat' : (user?.username === 'fajrur' ? 'Fajrur' : user?.username))}
                  </p>
                  <p className="text-xs opacity-60 line-clamp-1 break-words mt-0.5">
                    {user?.role && ROLE_PRESETS[user.role] ? ROLE_PRESETS[user.role].name : (user?.username === 'gpsttiaa' ? 'Administrator' : 'Pengguna')}
                  </p>
                </div>
              </div>
      ...
      <main className="flex-1 flex flex-col relative overflow-hidden bg-slate-50 dark:bg-slate-900 w-full">
        <header className="h-auto md:h-16 py-3 md:py-0 bg-white dark:bg-slate-800 border-b border-slate-200 dark:border-slate-700 flex flex-col md:flex-row items-start md:items-center justify-between px-4 md:px-6 shrink-0 gap-3 md:gap-0">
            ...
            <h2 className="font-semibold text-base md:text-lg text-slate-800 dark:text-slate-100 truncate">
              {viewingProfileId && canViewProfile ? "Profil Anggota" : (
                <>
                  ...
                  {activeTab === 'reports_umum' && "Laporan Kebaktian Umum"}
                  {activeTab === 'reports_remaja' && "Laporan Kebaktian Remaja"}
                  {activeTab === 'reports_anak' && "Laporan Kebaktian Anak (Sekolah Minggu)"}
                  {activeTab === 'reports' && "Ringkasan Kebaktian Terpadu (Semua Ibadah)"}
                  {activeTab === 'finance' && "Manajemen Keuangan Terintegrasi"}
                  {activeTab === 'stats' && "Statistik Jemaat"}
                  {activeTab === 'map' && "Pemetaan Jemaat"}
                  {activeTab === 'worship' && "Tema Ibadah"}
                  {activeTab === 'users' && "Manajemen Akun & Hak Akses"}
          ...
          {viewingProfileId && canViewProfile ? (
            ...
            <div className="flex flex-wrap items-center gap-2 md:gap-3 w-full md:w-auto justify-end">
              ...
              {(canImportData || canExportData || canCreateMember) && (
        ...
        <div className={`flex-1 flex flex-col overflow-hidden min-h-0 ${activeTab === 'overview' ? '' : 'p-4 sm:p-6 gap-4 sm:gap-6'}`}>
              ...
              <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1)] flex flex-col min-h-0">
                ...
                <div className="flex items-center justify-between px-1 mb-1">
                  <div className="flex items-center gap-1.5 text-[11px] text-slate-500 dark:text-slate-400">
                    ...
                    <span>Klik pada baris jemaat untuk membuka profil & riwayat lengkap.</span>
                  </div>
                </div>
                        ...
                        paginatedMembers.map((member, idx) => {
                          ...
                          return (
                            <tr 
                              ...
                              className="hover:bg-blue-50/75 dark:hover:bg-slate-800/80 transition-colors group cursor-pointer"
                              title="Klik baris untuk melihat profil lengkap jemaat"
                            >
                              <td className="p-3 text-slate-700 dark:text-slate-300 font-medium">{globalIdx}</td>
                              <td className="p-3">
                                <span className="font-mono text-blue-600 dark:text-blue-400 font-bold group-hover:underline">
                                  ...
                                  <div className="flex justify-end gap-2">
                                    ...
                                    {canDeleteMember && (
                                      <button
                                        onClick={(e) => {
                                          e.stopPropagation();
                                          member.id && handleDeleteClick(member.id);
                                        }}
          ...
          {activeTab === 'reports_umum' && (
            <WeeklyReportsPanel 
              initialCategory="Umum" 
              onCategoryChange={(cat) => setActiveTab(cat === 'Semua' ? 'reports' : cat === 'Umum' ? 'reports_umum' : cat === 'Remaja' ? 'reports_remaja' : 'reports_anak')} 
            />
          )}

          {activeTab === 'reports_remaja' && (
            <WeeklyReportsPanel 
              initialCategory="Remaja" 
              onCategoryChange={(cat) => setActiveTab(cat === 'Semua' ? 'reports' : cat === 'Umum' ? 'reports_umum' : cat === 'Remaja' ? 'reports_remaja' : 'reports_anak')} 
            />
          )}

          {activeTab === 'reports_anak' && (
            <WeeklyReportsPanel 
              initialCategory="Anak" 
              onCategoryChange={(cat) => setActiveTab(cat === 'Semua' ? 'reports' : cat === 'Umum' ? 'reports_umum' : cat === 'Remaja' ? 'reports_remaja' : 'reports_anak')} 
            />
          )}
          ...
          {activeTab === 'settings' && (
            <div className="flex-1 bg-white dark:bg-slate-800 rounded-xl border border-slate-200 dark:border-slate-700 shadow-[0_10px_25px_-5px_rgba(0,0,0,0.1),0_8px_10px_-6px_rgba(0,0,0,0.1)] p-6 overflow-y-auto">
              <div className="max-w-xl mx-auto space-y-8">
                <div>
                  <h2 className="text-xl font-bold text-slate-800 dark:text-slate-100 mb-2">Pengaturan Sistem</h2>
                  <p className="text-sm text-slate-500 dark:text-slate-400 mb-6">Kelola preferensi akun, mode akses, dan database GEPEKRIS TRETES.</p>
                </div>
                
                <div className="space-y-4">
                  <div className="flex items-center justify-between border-b border-slate-100 dark:border-slate-700 pb-2">
                    <h3 className="text-sm font-bold uppercase tracking-wider text-slate-400">Status Akses & Akun</h3>
                    {canManageUsers && (
                      <button 
                        onClick={() => handleTabClick('users')}
                        className="text-xs font-semibold text-blue-600 dark:text-blue-400 hover:underline flex items-center gap-1 cursor-pointer"
                      >
                        <ShieldCheck className="w-3.5 h-3.5" /> Kelola Semua Akun &rarr;
                      </button>
                    )}
                  </div>

                  <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 p-4 bg-slate-50 dark:bg-slate-800/80 rounded-lg border border-slate-200 dark:border-slate-700">
                    <div className="min-w-0 flex-1 pr-4">
                      <div className="flex items-center gap-2">
                        ...
                        {isDirectAccessMode && (
                          <span className="px-2 py-0.5 text-[11px] font-bold bg-emerald-100 text-emerald-800 dark:bg-emerald-900/60 dark:text-emerald-300 rounded">
                            Mode Langsung (Bebas Akun)
                          </span>
                        )}
                      </div>
                      <p className="text-xs text-slate-500 dark:text-slate-400 line-clamp-1 mt-0.5">
                        Username ID: <span className="font-mono text-slate-600 dark:text-slate-300 font-semibold">{user?.username}</span>
                      </p>
                    </div>
                    <span className="px-3 py-1 bg-blue-100 dark:bg-blue-900/50 text-blue-800 dark:text-blue-300 text-xs font-bold rounded-full w-max shrink-0">
                      {user?.role && ROLE_PRESETS[user.role] ? ROLE_PRESETS[user.role].name : 'Administrator'}
                    </span>
                  </div>
                  ...
                  <div className="flex flex-wrap gap-2 pt-1">
                    <button
                      onClick={async () => {
                        if (window.confirm("Hapus akun aktif ini dari sistem dan aktifkan Mode Akses Langsung (Tanpa Akun) di browser ini?")) {
                          await deleteCurrentAccount();
                          addToast("Akun dihapus & Mode Akses Langsung aktif!", "success");
                        }
                      }}
                      className="inline-flex items-center gap-1.5 px-3 py-2 bg-red-50 hover:bg-red-100 text-red-700 dark:bg-red-950/40 dark:hover:bg-red-900/50 dark:text-red-300 border border-red-200 dark:border-red-800 rounded-lg text-xs font-semibold transition-all cursor-pointer"
                    >
                      <Trash2 className="w-3.5 h-3.5" />
                      Hapus Akun & Aktifkan Akses Langsung
                    </button>
                  </div>
                </div>
        ...
        <div className="text-center p-3 text-xs font-medium text-slate-500 dark:text-slate-400 border-t border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shrink-0 w-full z-10 transition-colors">
          Kontak: <a href="mailto:sekretariat@gepekristretes.org" className="text-blue-500 hover:underline">sekretariat@gepekristretes.org</a> | Bank BCA: <span className="font-mono bg-slate-100 dark:bg-slate-700 px-1 py-0.5 rounded">123-456-7890</span> a.n. GEPEKRIS TRETES
        </div>
      </main>
      ...
      {activeTab === 'members' && canCreateMember && (
        <button
          onClick={() => {
