...
export default function BulkEntryModal({ isOpen, onClose, onSuccess, members = [] }: BulkEntryModalProps) {
  ...
  const handleSave = async () => {
    ...
    const generatedNomorList = validRows.map(row => {
      let nomorAnggota = "";
      if (row.no_urut && row.tahun) nomorAnggota = `${row.no_urut.trim()}/GJS/${row.tahun.trim()}`;
    ...
    try {
      ...
      for (let i = 0; i < validRows.length; i += chunkSize) {
        ...
        for (const row of chunk) {
          ...
          let nomorAnggota = "";
          if (row.no_urut && row.tahun) nomorAnggota = `${row.no_urut}/GJS/${row.tahun}`;
      ...
      <div className="flex-1 overflow-auto bg-slate-100/50 dark:bg-slate-900 p-6">
        ...
        <div className="mt-6 flex flex-wrap gap-4 items-center">
          ...
          <div className="flex items-center gap-2 text-sm text-slate-500 dark:text-slate-400 ml-auto bg-amber-50 dark:bg-amber-900/20 text-amber-700 dark:text-amber-400 px-3 py-1.5 rounded-lg border border-amber-200 dark:border-amber-900/50">
            ...
            <span>Format No Anggota akan otomatis tergabung: <strong className="font-mono">NO_URUT / GJS / THN_AGT</strong></span>
          </div>
        </div>
      </div>
