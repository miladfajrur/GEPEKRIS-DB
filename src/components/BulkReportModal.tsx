...
export default function BulkReportModal({ isOpen, onClose, onSuccess }: BulkReportModalProps) {
  ...
  const handleKeyDown = (e: React.KeyboardEvent<HTMLElement>, index: number, field: string) => {
    if (e.key === 'Enter') {
      e.preventDefault();
      const colNames = ['tanggal_ibadah', 'nama_ibadah', 'kehadiran_dewasa', 'kehadiran_pemuda', 'kehadiran_anak', 'persembahan_umum', 'perpuluhan', 'diakonia', 'pemasukan_lainnya', 'pengeluaran', 'keterangan'];
  ...
  const handleSave = async () => {
    ...
    try {
      for (const row of validRows) {
        ...
        const parsedPersembahan = parseRupiah(row.persembahan_umum);
        const parsedPerpuluhan = parseRupiah(row.perpuluhan);
        const parsedDiakonia = parseRupiah(row.diakonia);
        const parsedLainnya = parseRupiah(row.pemasukan_lainnya);
        const totalPem = parsedPersembahan + parsedPerpuluhan + parsedDiakonia + parsedLainnya;

        // Auto determine category from name
        const lowerName = row.nama_ibadah.toLowerCase();
        let cat: "Umum" | "Remaja" | "Anak" | "Lainnya" = "Umum";
        if (lowerName.includes("anak") || lowerName.includes("sekolah minggu") || lowerName.includes("sm ") || lowerName.endsWith("sm")) {
          cat = "Anak";
        } else if (lowerName.includes("remaja") || lowerName.includes("pemuda") || lowerName.includes("youth")) {
          cat = "Remaja";
        }
        ...
        const reportData: WeeklyReport = {
          ...
          kehadiran_pemuda: cat === "Anak" ? 0 : (Number(row.kehadiran_pemuda) || 0),
          kehadiran_anak: cat === "Remaja" ? 0 : (Number(row.kehadiran_anak) || 0),
          persembahan_umum: parsedPersembahan,
          persembahan_anak: cat === "Anak" ? parsedPersembahan : 0,
          persembahan_remaja: cat === "Remaja" ? parsedPersembahan : 0,
          perpuluhan: parsedPerpuluhan,
          diakonia: parsedDiakonia,
          pemasukan_lainnya: parsedLainnya,
          ...
          <table className="w-full text-left whitespace-nowrap min-w-max border-collapse">
            <thead>
              <tr className="bg-slate-50 dark:bg-slate-900/50 border-b border-slate-200 dark:border-slate-700">
                ...
                <th className="px-3 py-3 text-xs font-bold text-rose-600 dark:text-rose-400 uppercase tracking-wider border-l border-slate-200 dark:border-slate-700 min-w-[120px]">Pengeluaran (Rp)</th>
            ...
            <tbody className="divide-y divide-slate-100 dark:divide-slate-800/60">
              {rows.map((row, index) => (
                <tr key={row._localId} className="hover:bg-blue-50/50 dark:hover:bg-slate-800/80 group">
                  ...
                  <td className="px-2 py-1 border-l border-slate-100 dark:border-slate-800 bg-white dark:bg-slate-800 group-hover:bg-transparent transition-colors">
                    <input type="text" value={formatRupiah(row.pengeluaran)} onChange={(e) => handleChange(row._localId, 'pengeluaran', e.target.value)} onKeyDown={(e) => handleKeyDown(e, index, 'pengeluaran')} data-col="pengeluaran" data-rowindex={index} className={`${inputClass} text-right font-mono min-w-[120px] text-rose-600 dark:text-rose-400`} placeholder="0" />
                  </td>
