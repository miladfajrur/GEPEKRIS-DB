...
export default function MisiKaltaraFinancePanel({ collectionName = "misi_finance" }: { collectionName?: string }) {
  ...
  const handleExportPDF = async () => {
    try {
      ...
      doc.text(`Periode Cetak: ${yearFilter === "Kustom" ? `${startDateFilter ? formatDateDDMMYYYY(startDateFilter) : '...'} s/d ${endDateFilter ? formatDateDDMMYYYY(endDateFilter) : '...'}` : yearFilter}  |  Tanggal: ${formatDateDDMMYYYY(new Date().toISOString())}`, 40, 68);
