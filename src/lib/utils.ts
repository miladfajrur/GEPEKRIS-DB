...
export const getWorshipCategory = (report: { kategori_ibadah?: string; nama_ibadah?: string }): "Umum" | "Remaja" | "Anak" | "Lainnya" => {
  if (report.kategori_ibadah) {
    if (["Umum", "Remaja", "Anak", "Lainnya"].includes(report.kategori_ibadah)) {
      return report.kategori_ibadah as "Umum" | "Remaja" | "Anak" | "Lainnya";
    }
  }
  const name = (report.nama_ibadah || "").toLowerCase();
  if (name.includes("anak") || name.includes("sekolah minggu") || name.includes("sm ") || name.endsWith("sm") || name.includes("kids") || name.includes("balita")) {
    return "Anak";
  }
  if (name.includes("remaja") || name.includes("pemuda") || name.includes("youth") || name.includes("teen") || name.includes("kaum muda")) {
    return "Remaja";
  }
  if (name.includes("doa") || name.includes("pa ") || name.includes("pemahaman alkitab") || name.includes("wanita") || name.includes("pria") || name.includes("kaum bapak") || name.includes("kaum ibu")) {
    return "Lainnya";
  }
  return "Umum";
};
